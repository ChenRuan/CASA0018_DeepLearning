...
/* Includes ---------------------------------------------------------------- */
#include <CASA0018_ChenRuan_inferencing.h>
...
void setup() {
  ...
  while (!Serial)
    ;
  Serial.println("Plant Health Detection based on Edge Impulse");
...
void loop() {
  ...
  while (stop_inferencing == false) {
    ...
    if (result.classification[0].value > 0.53) {  // 假设Healthy是第一个类别
      digitalWrite(A6, HIGH);
      digitalWrite(A7, LOW);
      digitalWrite(D12, LOW);
    } else if (result.classification[1].value > 0.53) {  // 假设Unhealthy是第二个类别
      digitalWrite(A6, LOW);
      digitalWrite(A7, LOW);
      digitalWrite(D12, HIGH);
    } else {
